! function (t, e) {
    ...
    e.a = {
      ...
      methods: {
        ...
        navigateToTab: function (t) {
          ...
          e.$emit('on-tab-change',e);
          return t <= this.maxStep
        },
        nextTab: function (isFinish) {
          var t = this;
          var e = function () {
              
              if(isFinish){
                // t.activeTabIndex = (t.tabCount - 1);
                // t.navigateToTab(t.tabCount - 1);
                t.$emit("on-complete",t);
              }else{
                (t.changeTab(t.activeTabIndex, t.activeTabIndex + 1), t.afterTabChange(t.activeTabIndex))
                t.$emit("on-next",t);
              }
            };
        ...
        scrollToActiveTab:function(){
          var scrollElement = $(this.$el).find('ul[role="tablist"].wizard-nav');
          var tabPanel = $(this.$el).find('div.wizard-tab-content').find('.wizard-tab-container');
              tabPanel.addClass('is-animated--active');
          setTimeout(function(){
            if(scrollElement){
              scrollElement = $(scrollElement[0]);
              var parentPos = scrollElement.offset(),
                  scrollLeftPos = scrollElement.scrollLeft(),
                  activeElement = scrollElement.find('li.active'),
                  activeId = activeElement.find('div[role="tab"]').attr('aria-controls'),
                  activePanal = tabPanel.filter(function(panalIndex,panal){
                    var pId = panal.getAttribute('id');
                    return (pId == activeId)
                  });

              if(activePanal){ 
                tabPanel.removeClass('is-animated--active');
                $(activePanal[0]).addClass('is-animated--active'); 
              }
              
              if(activeElement){
                var childPos = activeElement.offset();
                var childOffset = {
                  top: childPos.top - parentPos.top,
                  left: childPos.left - parentPos.left
                }
                scrollElement.stop().animate({
                  scrollLeft: (scrollLeftPos + childOffset.left - 300)
                },500, 'linear');
              }           
            }
          },0);
      ...
      mounted: function () {
        ...
        if(scrollElement){
          ...
          scrollElement[0].addEventListener('wheel', function(e){
          dir = e.deltaY > 0 ? '-=' : '+=';
          $(scrollElement[0]).stop().animate({
            scrollLeft: dir + amt
          },500, 'linear');
          }, {
          capture: true,
  ...
  function (t, e, a) {
    ...
    var n = function () {
      ...
      return a("li", {
        ...
        class: {
          checked: t.tab.checked, square_shape: t.isStepSquare, tab_shape: t.isTabShape, has_error: t.tab.validationError
        },
        style: [
  ...
  function (t, e, a) {
    ...
    var n = function () {
      ...
      }, [t._t("custom-buttons-right", null, null, t.slotProps), t._v(" "),
       ...
       !t.isLastStep ? a("span", 
          {
                ...
                on: {
                  click: function(){t.nextTab(false)},
                  keyup: function (e) {
                    if (!("button" in e) && t._k(e.keyCode, "enter", 13, e.key)) return null;
                    t.nextTab(false,e)
                  }
